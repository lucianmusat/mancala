...
class TestGame:
    ...
    def basic_setup(self):
        ...
        return board, strategy, player1, player2

    def test_player_id(self, basic_setup):
        _, _, player1, player2 = basic_setup
    ...
    def test_player_basic_move(self, basic_setup):
        _, strategy, player1, player2 = basic_setup
        assert strategy.move_pit(player1.id, 0) == MoveResult.InOwnBigPit
    ...
    def test_player_move_empty_pit(self, basic_setup):
        board, strategy, player1, _ = basic_setup
    ...
    def test_player_move_twice(self, basic_setup):
        board, strategy, player1, player2 = basic_setup
        board.player_one_pits = [0, 7, 7, 7, 7, 7]
        board.player_one_big_pit = 1
        board.player_two_pits = [6, 6, 6, 6, 6, 6]
        board.player_two_big_pit = 0
        assert strategy.move_pit(player1.id, 1) == MoveResult.Valid
    ...
    def test_player_land_on_empty(self, basic_setup):
        ...
        assert strategy.move_pit(player1.id, 0) == MoveResult.Valid
    ...
    def test_land_on_empty_with_moves(self, basic_setup):
        ...
        assert strategy.move_pit(player1.id, 0) == MoveResult.InOwnBigPit
        assert strategy.move_pit(player1.id, 1) == MoveResult.Valid
        assert strategy.move_pit(player2.id, 0) == MoveResult.Valid
        assert strategy.move_pit(player1.id, 0) == MoveResult.Valid
        assert player1.pits == [0, 0, 8, 8, 8, 8]
        assert player1.big_pit == 10
        assert player2.pits == [0, 8, 7, 7, 0, 7]
    ...
    def test_overflow_player1(self, basic_setup):
        ...
        board.player_two_pits = [6, 6, 6, 6, 6, 6]
        board.player_two_big_pit = 0
        assert strategy.move_pit(player1.id, 4) == MoveResult.Valid
    ...
    def test_overflow_player2(self, basic_setup):
        ...
        assert strategy.move_pit(player2.id, 4) == MoveResult.Valid
    ...
    def test_land_in_big_pit(self, basic_setup):
        ...
        board.player_two_big_pit = 1
        assert strategy.move_pit(player1.id, 4) == MoveResult.InOwnBigPit
    ...
    def test_player(self, basic_setup):
        board, _, player1, player2 = basic_setup
        assert player1.id == 1
        assert player2.id == 2
